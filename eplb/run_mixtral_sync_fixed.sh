#!/bin/bash
# Mixtral-8x7B EPLB Sync 模式固定配置实验脚本 (替代 test_mixtral_eplb_scheduler.sh)
#
# 目的: 在可复现的硬件/软件配置下，验证 sync EPLB + PCIe Scheduler 的增量收益
#
# 与旧脚本的关键差异:
#   1. **固定 GPU**: 强制使用 GPU0-3 (同一 NUMA 节点，同一 PCIe root complex)
#      不再 auto-select，以消除 PCIe 拓扑带来的运行间方差
#   2. **真 sync**: use_async 硬改为 false (旧脚本 bug: 硬写 true)
#   3. **参数归一化**: step_interval=200 (旧: 50，rearrange 过频繁)
#                    num_gpu_blocks=4000 (旧: 2500，block 不足)
#                    log_balancedness=true (开启负载均衡探针)
#   4. **多轮默认 3 次**: 便于看方差
#
# 实验组:
#   G1: Reference: EP + Offload + Prefetch (无 EPLB)
#   G2: Baseline:  EP + Offload + Prefetch + EPLB (无 sched)
#   G3: Sched:     +PCIe Sched (无 EPLB phase)      [分离 sched 与 eplb_phase 贡献]
#   G4: Full:      +PCIe Sched + EPLB Phase
#
# 用法:
#   ./run_mixtral_sync_fixed.sh                          # 跑所有 QPS, 3 轮
#   ./run_mixtral_sync_fixed.sh --qps-list "2.0"         # 只跑 QPS=2.0
#   ./run_mixtral_sync_fixed.sh --groups g2,g4           # 只跑 G2 和 G4
#   ./run_mixtral_sync_fixed.sh --rounds 1               # 快速冒烟
#   ./run_mixtral_sync_fixed.sh --gpus 4,5,6,7           # 用 NUMA1 那组
...
# ============================================================
# Defaults - 针对 sync 模式调优
# ============================================================
MODEL_PATH="${MODEL_PATH:-/lpai/models/mistralai__mixtral-8x7b-instruct-v0_1/24-08-19-1318}"
API_PORT="${API_PORT:-8000}"
EP_SIZE=4
GPU_LIST="0,1,2,3"                # 同 NUMA 节点 (NUMA 0), 由 nvidia-smi topo -m 确认
GPU_MEM_UTIL=0.5
NUM_GPU_BLOCKS=4000               # 关键: 消除 Block allocation failed 抖动
MAX_NUM_SEQS=32
MAX_MODEL_LEN=4096
KV_OFFLOADING_SIZE=20
EPLB_STEP_INTERVAL=200            # 关键: 从 50 调到 200, rearrange 频率 × 1/4
...
RESULTS_ROOT="$SCRIPT_DIR/results/mixtral_sync_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$RESULTS_ROOT"
PIDFILE="/tmp/vllm_mixtral_sync_${API_PORT}_$$.pid"
...
# ============================================================
# Start vLLM — SYNC 模式 (use_async=false)
# ============================================================
start_vllm() {
    local label="$1"
    local enable_eplb="$2"
    ...
    log "Starting vLLM [$label] eplb=$enable_eplb(sync) sched=$enable_pcie_sched phase=$enable_eplb_phase"
    stop_our_vllm
    ...
    if [[ "$enable_eplb" -eq 1 ]]; then
        # 关键: use_async=false (真 sync), log_balancedness=true (探针)
        CMD_ARGS+=(
            --enable-eplb
            --eplb-config "{\"step_interval\": $EPLB_STEP_INTERVAL, \"num_redundant_experts\": 0, \"use_async\": false, \"log_balancedness\": true, \"log_balancedness_interval\": 10}"
        )
    fi
...
get_group_config() {
    case "$1" in
        g1) echo "prefetch 0 0 0" ;;  # Reference: no EPLB, no sched
        g2) echo "prefetch 1 0 0" ;;  # Baseline: +EPLB, no sched
...
# ============================================================
# Post-run health inspection for a single vLLM log
# ============================================================
inspect_run_health() {
    local vllm_log="$1"
    local label="$2"
...
generate_report() {
    ...
    {
        echo "# Mixtral-8x7B EPLB Sync (Fixed) Report"
        echo ""
        ...
        echo "| EPLB mode | **sync** (use_async=false) |"
...
if [[ $DRY_RUN -eq 1 ]]; then
    echo "========================================"
    echo "DRY RUN — Mixtral Sync Experiment Matrix"
    echo "========================================"
    echo "EPLB mode: SYNC (use_async=false)"
    echo "GPUs: $GPU_LIST"
...
# Main loop
# ============================================================
log_section "Mixtral-8x7B EPLB SYNC Fixed Experiments"
log "Model:      $MODEL_PATH"
log "EP size:    $EP_SIZE"
log "EPLB mode:  SYNC (use_async=false)"
