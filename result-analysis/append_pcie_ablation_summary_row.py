#!/usr/bin/env python3
"""
将一次 4 组消融实验的汇总指标追加到 TSV 文本表（每组一行，共 4 行）。

表头首次自动写入；之后每次运行追加 4 行（G0/G1/G2/G3）。
便于粘贴到 Excel / Google Sheets。

列：实验配置 + 组别 + TTFT + TPOT + PCIe 带宽 + 缓存命中率
"""
...
# 组别定义：(group_name, suffix, description)
GROUPS = [
    ("G0", "g0_no_prefetch", "No Sched, No Prefetch"),
    ("G1", "g1_prefetch_only", "No Sched, Prefetch"),
    ("G2", "g2_sched_no_phase", "Sched, No Phase-Aware"),
    ("G3", "g3_full_sched", "Sched + Phase-Aware"),
]
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Append 4 ablation rows (G0-G3) to a TSV summary table")
    ...
    for group_name, suffix, group_desc in GROUPS:
        ...
        row = build_row(args, model, group_name, group_desc, rows, pcie)
        lines.append(row)
    ...
    print(f"Appended {len(GROUPS)} rows to {table_path}")
